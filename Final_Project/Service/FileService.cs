...
namespace Final_Project.Service
{
    public class FileService : IFileService
    {
        ...
        public async Task<(bool status, string messasge, string path)> HandleUserProfilePhoto(IFormFile file, string? OldPath)
        {
            var permittedExtensions = new[] { ".jpg", ".jpeg", ".png", ".webp" };
            var extension = Path.GetExtension(file.FileName).ToLowerInvariant();
            ...
            // Generate unique filename
            var fileName = $"{Guid.NewGuid()}{extension}";
            var uploadPath = Path.Combine(_env.WebRootPath ?? "wwwroot", "UserProfilePic");

            if (!Directory.Exists(uploadPath))
                Directory.CreateDirectory(uploadPath);

            var fullPath = Path.Combine(uploadPath, fileName);
            var relativePath = Path.Combine("/UserProfilePic", fileName).Replace("\\", "/");
